...
const Reports = () => {
    ...
    return (
        <div className="flex-1 overflow-y-auto p-6 lg:p-10 space-y-6 animate-in fade-in duration-500">
            {/* Header */}
            <div className="bg-[#1e293b] rounded-[2.5rem] p-8 lg:p-10 text-white relative overflow-hidden shadow-2xl">
                <div className="relative z-10 flex flex-col sm:flex-row justify-between gap-4">
                    <div>
                        <h1 className="text-2xl lg:text-4xl font-bold tracking-tight mb-1">
                            Business Reports <span className="text-purple-400">/ Ripoti</span>
                        </h1>
                        <p className="text-gray-400 text-sm">Intelligence at a glance — sales, products, inventory, and profit.</p>
                    </div>
                    {/* Date Range Picker */}
                    <div className="flex items-center gap-2 bg-white/10 rounded-2xl p-3 shrink-0 self-start">
                        ...
                        <button onClick={() => fetchReport(activeTab)}
                            className="ml-1 p-1.5 bg-white/10 hover:bg-white/20 rounded-xl transition-colors">
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
                        ...
                        return (
                            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                                className={`flex items-center gap-2 px-5 py-2.5 rounded-2xl text-sm font-bold transition-all shadow-lg
                                    ${activeTab === tab.id ? `${c.pill}` : 'bg-white text-gray-500 border border-gray-200 hover:border-gray-300 shadow-none'}`}>
                ...
                <div className="flex gap-2 flex-wrap">
                    {activeTab === 'sales' && (
                        <>
                            <button onClick={() => downloadFile('/export/sales-csv', dateRange)}
                                className="flex items-center gap-1.5 px-4 py-2 bg-white border border-gray-200 text-gray-600 hover:bg-green-50 hover:border-green-300 hover:text-green-700 rounded-xl text-xs font-bold transition-all">
                                <Sheet className="w-3.5 h-3.5" /> CSV
                            </button>
                            <button onClick={() => downloadFile('/export/report-pdf', dateRange)}
                                className="flex items-center gap-1.5 px-4 py-2 bg-white border border-gray-200 text-gray-600 hover:bg-red-50 hover:border-red-300 hover:text-red-700 rounded-xl text-xs font-bold transition-all">
                                <FileText className="w-3.5 h-3.5" /> PDF Report
                            </button>
                        </>
                    )}
                    {activeTab === 'products' && (
                        <button onClick={() => downloadFile('/export/top-products-csv', dateRange)}
                            className="flex items-center gap-1.5 px-4 py-2 bg-white border border-gray-200 text-gray-600 hover:bg-green-50 hover:border-green-300 hover:text-green-700 rounded-xl text-xs font-bold transition-all">
                            <Sheet className="w-3.5 h-3.5" /> CSV
                        </button>
                    )}
                    {activeTab === 'inventory' && (
                        <button onClick={() => downloadFile('/export/inventory-csv')}
                            className="flex items-center gap-1.5 px-4 py-2 bg-white border border-gray-200 text-gray-600 hover:bg-green-50 hover:border-green-300 hover:text-green-700 rounded-xl text-xs font-bold transition-all">
            ...
            {loading ? (
                ...
                <>
                    ...
                    {activeTab === 'sales' && tabData && (
                        <div className="space-y-6">
                            ...
                            <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-gray-100">
                                <h3 className="font-bold text-gray-800 mb-4 text-sm uppercase tracking-wider">Daily Revenue Trend</h3>
                                {(tabData.daily?.length ?? 0) === 0 ? (
                                    ...
                                    <ResponsiveContainer width="100%" height={280}>
                                        <AreaChart data={tabData.daily} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                                            <defs>
                    ...
                    {activeTab === 'products' && tabData && (
                        <div className="space-y-6">
                            <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-gray-100">
                                <h3 className="font-bold text-gray-800 mb-4 text-sm uppercase tracking-wider">Top 15 Products by Units Sold</h3>
                                {tabData.length === 0 ? (
                                    <p className="text-center text-gray-400 italic py-12 text-sm">No sales in this date range.</p>
                                ) : (
                                    <ResponsiveContainer width="100%" height={320}>
                                        <BarChart data={tabData} layout="vertical" margin={{ left: 100, right: 30, top: 5, bottom: 5 }}>
                                    ...
                                    <tbody className="divide-y divide-gray-50">
                                        {tabData.map((p, i) => (
                                            <tr key={p.id} className="hover:bg-gray-50/50">
                                                ...
                                                <td className="px-4 py-3 text-right font-black text-blue-700">{fmt(p.total_sold)}</td>
                                                <td className="px-6 py-3 text-right font-black text-green-700">{fmtCur(p.total_revenue)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden">
                                <div className="p-6 border-b border-gray-100">
                                    <h3 className="font-bold text-gray-800 text-sm uppercase tracking-wider">Stock Value by Category</h3>
                                </div>
                                <table className="w-full text-sm">
                                    <thead><tr className="border-b border-gray-100">
                                    ...
                                    <tbody className="divide-y divide-gray-50">
                                        {(tabData.by_category ?? []).map((c, i) => (
                                            <tr key={i} className="hover:bg-amber-50/30">
                                                ...
                                                <td className="px-4 py-3 text-right font-semibold text-gray-700">{fmt(c.total_units)}</td>
                                                <td className="px-4 py-3 text-right text-blue-600 font-bold">{fmtCur(c.stock_value_cost)}</td>
                                                <td className="px-6 py-3 text-right text-green-700 font-black">{fmtCur(c.stock_value_retail)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                    ...
                    {activeTab === 'profit' && tabData && (
                        <div className="space-y-6">
                            ...
                            {/* Profit Gauge */}
                            <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-gray-100">
                                <h3 className="font-bold text-gray-800 mb-4 text-sm uppercase tracking-wider">Revenue vs Cost Breakdown</h3>
                                <div className="flex gap-6 items-center mb-6">
                                    ...
                                    <span className="font-black text-green-700 shrink-0 text-lg">{tabData.gross_margin}% margin</span>
                                </div>
                                <div className="grid grid-cols-3 gap-4 text-center">
                                    <div className="p-4 bg-green-50 rounded-2xl border border-green-100">
                                        <p className="text-xs font-bold text-green-800 uppercase mb-1">Revenue</p>
                                        <p className="text-lg font-black text-green-700">{fmtCur(tabData.revenue)}</p>
                                    </div>
                                    <div className="p-4 bg-blue-50 rounded-2xl border border-blue-100 flex flex-col items-center justify-center">
                                        ...
                                        <p className="text-lg font-black text-blue-700">- {fmtCur(tabData.cogs)}</p>
                                    </div>
                                    <div className="p-4 bg-purple-50 rounded-2xl border border-purple-100">
                                        <p className="text-xs font-bold text-purple-800 uppercase mb-1">Gross Profit</p>
                                        <p className="text-lg font-black text-purple-700">= {fmtCur(tabData.gross_profit)}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            {(tabData.monthly?.length ?? 0) > 0 && (
                                <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-gray-100">
                                    <h3 className="font-bold text-gray-800 mb-4 text-sm uppercase tracking-wider">Monthly Revenue</h3>
                                    <ResponsiveContainer width="100%" height={240}>
                                        <BarChart data={tabData.monthly} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                                            <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
